...
class database():
	foldernames = ['box0','box1','box2','box3','box4','box5','box6','box7']
	sx = 12
	sy = 12
	...
	def greet(self):
		page,x,y = database.id2crd(self.id)
		theType = self.getCurrentType()
		print('Current:', '[',theType,page,']',database.foldernames[page],'\t','\tx:',x,'\ty:',y,'\tid:',self.id,'name:',database.id2shortname(self.id))

	def setCurrentType(self, curType):
...
def wait4histogram(xvid, video, seconds, db, name=''):
	...
	if (seconds > 0):
		while(True):
			...
			if (d >= seconds):
				ok = processor1000.check_before_processing(img)
				if (ok):
			...
			if (key == ord('q')):
				db.greet()
				exit(1)
			if (key > 0 and chr(key) in 'tcds'): # triangles, clubs, dots, stripes
				db.setCurrentType(chr(key))
				db.greet()
...
def main():
	...
	while True:
		...
		##########
		img = xvid.capture(video)
		p = processor1000.process_cam(img, d.getCurrentType())
		p.id = database.id2filename(id)
		#p.show(True)
		p.type = d.getCurrentType()
		p.save()
